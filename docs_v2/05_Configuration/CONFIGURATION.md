...
**When a change takes effect (#143).** Everything parsed into `RuntimeSettings` is read **once, at process start** (the FastAPI lifespan) and then carried by the components built from it. Changing such a variable in a running process has no effect — `heroku config:set` restarts the dyno, so it applies there, but a locally started `uvicorn` needs a restart. On the **auth path** nothing is snapshotted: `WEB_AUTH_TOKEN`, `WEB_AUTH_USER`, `WEB_AUTH_PASS`, `WEB_ALLOW_UNAUTHENTICATED`, `WEB_REQUIRE_HEADER_AUTH_WITH_COOKIE`, `WEB_ADMIN_COOKIE_EPOCH`, `WEB_ADMIN_COOKIE_TTL_SECONDS`, `AUTH0_DOMAIN`/`AUTH0_CLIENT_ID` and the signing-secret trio `WEB_SESSION_SECRET`/`SECRET_KEY`/`WEB_DEV_INSECURE_SECRET` are read per request — exactly the variables `web/auth.py` reads, which `tests/test_env_centralization.py` pins by name. A few reads outside that path are also per-call rather than snapshotted and are unaffected by #143: `DATABASE_URL` (the readiness probe's availability check), and `XDG_CACHE_HOME` plus the session secret where the file-based posted-state and the Instagram session store resolve their cache paths.
...
| `PUBLISH_CLAIM_TIMEOUT_SECONDS` | Budget for the publish-store claim (the `posted_platforms` + `acquire_lease` pair) at the start of a publishing run (PUB-047). **If the claim fails or exceeds this budget while `DATABASE_URL` is set, the run aborts and publishes nothing** — it no longer falls open and publishes without a lease. The CLI exits 1 and the run logs `publish_store_unavailable`; `POST /api/publish` returns 200 with `any_success=false` and an empty `results`, not a 500. No effect when no publish store is configured | 10.0 |
| `DB_CONNECT_TIMEOUT_SECONDS` | asyncpg connection timeout passed as `connect_args["timeout"]` (PUB-047). Bounds how long a new Postgres connection may take to establish | 10.0 |
| `DB_COMMAND_TIMEOUT_SECONDS` | asyncpg per-statement timeout passed as `connect_args["command_timeout"]` (PUB-047). Bounds every DB call, including `CaptionStore` calls that are not covered by `PUBLISH_CLAIM_TIMEOUT_SECONDS` | 30.0 |
