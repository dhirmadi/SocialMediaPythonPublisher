...
- **Problem**: The core CLI workflow currently performs de-duplication by downloading candidate images from S3-compatible storage and computing local SHA256 hashes, then comparing those hashes against a local `posted` state. For folders with many already-posted images, this leads to excessive network I/O, slow selection, and unnecessary cost, especially when the outcome is simply “no new images”.
- **Goals**:
  - Use storage-native metadata (e.g., ETag/content hash when available) to perform de-duplication as early as possible, minimizing full image downloads.
...
- **Current state**:
  - `WorkflowOrchestrator.execute`:
    - Lists images via the configured storage adapter, returning a list of filenames.
  ...
  - Storage adapter:
    - Uses S3-compatible storage and exposes `list_images`, `download_image`, `get_temporary_link`, `get_file_metadata`, and `archive_image`.
    - `list_images` currently returns only image filenames, discarding metadata such as ETag/content hash.
- **Constraints**:
  - Python `>=3.9,<4.0`.
...
- **Assumptions**:
  - Provider hashing metadata (ETag/content hash) is stable for a given object content and can be used as a logical content identifier across runs.
...
5. **Preview/dry/debug semantics**  
   - In preview/dry/debug modes:
     ...
     - It must not archive or mutate storage contents.
...
- Introduce a **metadata-aware listing** operation in the storage adapter that returns both filenames and their provider hash/ETag.
- Extend **posted state** to track both SHA256 and provider hash/ETag values in a single JSON file.
  ...
  - State file format:
    ...
    - New extended dict format:
      ```json
      {
        ...
        "storage_content_hashes": ["hash-1", "hash-2"]
      }
      ```
...
- **`publisher_v2.core.workflow.WorkflowOrchestrator`**
  - Selection flow changes:
    ...
    - When `select_filename` is **not** provided:
      ...
      2. Load posted SHA256 hashes and storage provider hash/ETag values.
...
- **After**:
  - `posted.json`:
    - May be:
      ...
      - A dict with:
        - `"hashes": [sha256...]`
        - `"storage_content_hashes": [provider_hash...]` (optional).
  - `utils.state` continues to expose `load_posted_hashes()` / `save_posted_hash()` for SHA256 and adds dedicated helpers for storage hashing metadata.
...
6. On successful archive:
   ...
   - Saves storage provider hash/ETag via `save_posted_content_hash`.
...
2. After analysis/caption/preview output:
   ...
   - No archive; storage contents remain unchanged.
...
- **Migration / Backfill**:
  ...
  - As new runs archive images, both SHA256 and storage provider hash/ETag entries will gradually accumulate.
- **Monitoring & Logging**:
...
- **Risks**
  - **Storage hashing semantics change**: could lead to false negatives/positives in dedup.
