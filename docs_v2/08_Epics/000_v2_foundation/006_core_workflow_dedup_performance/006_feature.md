<!-- docs_v2/08_Epics/08_01_Feature_Request/006_core-workflow-dedup-performance.md -->
...
## Summary
The core workflow previously performed image de-duplication by downloading candidate images from storage and computing local hashes, which did not scale efficiently as the image library grew.  
This feature introduces a revised deduplication and selection strategy that leverages storage metadata (e.g., ETag/content hash when available) and minimizes network I/O while keeping existing behavior and sidecar semantics intact.  
...
## Problem Statement
Today, `WorkflowOrchestrator` selects an image to post by listing images from the configured storage backend and then downloading them one by one to compute SHA256 hashes, skipping those that already appear in the local `posted` state.  
...
- Use storage metadata (e.g., ETag/content hash when available) to perform de-duplication as early as possible in the workflow.  
...
## User Stories
- As an operator, I want the CLI workflow to select a new image quickly even when the S3 library contains many previously posted files, so that routine runs remain fast over time.  
- As a maintainer, I want de-dup logic to use efficient metadata-based checks wherever possible, so that we minimize unnecessary object downloads.  
...
- Given an S3 prefix containing a mix of new and already-posted images, when the workflow runs, then it must avoid downloading more than a small number of images (ideally one) before selecting a new, unposted image.  
- Given an S3 prefix where all images have already been posted, when the workflow runs, then it must still correctly report "no new images" without downloading every file in the prefix.  
- Given the posted-hash state and S3 contents are consistent, when the workflow runs multiple times, then it must not select the same image twice until the state is cleared or reset.  
...
- The feature uses S3-compatible storage as the source of truth for images and sidecars.  
- Storage metadata used for deduplication (e.g., ETag/content hash when available) is assumed to be stable and suitable for deduplication across runs.  
...
- S3-compatible storage adapter for listing images and retrieving metadata.  
...
## Data Model / Schema
- The posted-hash state file has been extended to optionally store storage-level content hashes alongside existing SHA256 hashes while preserving legacy formats.  
...
- No new categories of data are stored beyond hashes/identifiers already present in storage and local state.  
...
## Risks & Mitigations
- Relying on storage metadata could introduce subtle drift if hashing semantics change — Mitigation: document assumptions and maintain a migration path back to local hashing if needed.  
...
- Edge cases with partially uploaded or renamed files — Mitigation: rely on storage APIs for consistent listing and metadata; handle errors and unexpected states gracefully.

## Open Questions
- Should we store both local SHA256 and storage content hash/ETag in posted state for a period of time? — Proposed answer: likely yes for a safe transition.  
...
- Documentation and any relevant diagrams are updated to reflect the new selection strategy and the use of storage hashing metadata in selection and state.

## Appendix: Source Synopsis
- Core workflow performance review identified image selection and dedup as a major latency source due to repeated downloads and hashing.  
- Architecture guidelines emphasize storage as source of truth and avoiding new data stores.  
- Earlier discussions proposed using storage hashing metadata and improving selection complexity while retaining existing sidecar and posting semantics.
