...
- Problem & context: The current web interface lists images from storage and performs AI analysis sequentially on every request, even when sidecars already exist. This leads to unnecessary latency and external API usage, especially when repeatedly requesting random images or re-analyzing known files.  
- Goals & Non-goals: This change introduces a short-lived in-memory cache for storage image listings, adds sidecar-first behavior for analysis in the web layer (reusing the canonical semantics from CR 001-001), and parallelizes safe storage calls in random/analyze flows, all while preserving existing HTTP contracts and auth semantics.
...
- Current behavior (affected parts):
  ...
  - Web endpoints in `web.app` call these service methods directly; all storage calls inside a request are performed serially.  
- Constraints inherited from the parent feature:
  - S3 remains the sole persistent store; no databases or new cache layers may be introduced.  
...
- Dependencies:
  - Storage adapter for listing images, obtaining temporary links, and downloading sidecars.  
...
- **CR1:** Add a short-lived, in-memory cache of the storage image list inside the web service layer, used by `/api/images/random` to avoid re-listing on every request when the underlying folder is unchanged.  
...
- **CR4:** Parallelize independent storage operations (e.g., temp link + sidecar download, or temp link + hash computation) within a single request using asyncio primitives, without changing external behavior.  
...
- Current:
  - Each `/api/images/random` request triggers a fresh `list_images` call and then downloads one image to compute its SHA256; all storage calls occur sequentially.  
...
- Proposed:
  ...
  - Use `asyncio.gather` in web service methods to perform independent storage operations concurrently where safe.
...
- Image list cache:
  ...
  - Stale cache entries may reference images removed from storage; in that case, downstream storage calls will raise `StorageError`/`FileNotFoundError`, which are already handled at the web layer. On error, the cache should be invalidated on the next request.  
- Sidecar cache:
...
- Main success path: `GET /api/images/random` (with cache):
  ...
  2. Service checks image-list cache:
     ...
     - If expired or empty, calls `list_images`, stores the list with a new expiry timestamp, and logs `image_list_cache_refresh`.  
