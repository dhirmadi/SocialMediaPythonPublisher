...
- Admin mode for a browser session is the signed, tenant/host-bound admin cookie (`pv2_admin`); a valid cookie alone satisfies `require_auth` by default (#91 SEC-3 decision b). Machine clients use HTTP auth (`WEB_AUTH_TOKEN` Bearer or `WEB_AUTH_USER`/`WEB_AUTH_PASS` Basic), which satisfies `require_auth` but NOT `require_admin`. **Since #137 the library routes AND the image routes call `require_admin` unconditionally**, so Bearer/Basic-only automation can no longer analyze, publish, keep, remove or delete: without Auth0 configured those routes answer 503, and without an admin cookie 403. **Auth0 is the only admin login** — the cookie is minted solely by the Auth0 callback, and no password login may be added back (#137). Set `WEB_REQUIRE_HEADER_AUTH_WITH_COOKIE=1` to require header auth alongside a cookie session. See `.claude/rules/web-security.md`, which is the source of truth for this model.
