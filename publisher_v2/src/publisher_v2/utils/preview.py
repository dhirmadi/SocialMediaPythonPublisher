...
def print_image_details(
    ...
    if already_posted:
        print("  Status:      ⚠️  Previously posted (would be skipped in real run)")
    elif is_new:
        print("  Status:      ✓ New (not previously posted)")
    else:
    ...
    for pub in publishers:
        if pub.is_enabled():
            ...
            if pub.platform_name == "telegram":
                print("     → Image will be resized to max 1280px width")
            elif pub.platform_name == "instagram":
                print("     → Image will be resized to max 1080px width")
                # Count hashtags
                ...
                print("     → FetLife formatting: no hashtags, ≤240 chars")
