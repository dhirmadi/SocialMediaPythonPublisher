...
Laws 2–4 aren't just discipline to remember inline — `/implement` enforces the role split
structurally by delegating to isolated-context subagents in `.claude/agents/`:
`test-engineer` writes the failing test and is not given the implementation task;
`developer` writes the code and is not allowed to edit the test files it's working
against; `code-reviewer` (read-only) checks the result afterward. Prefer that delegation
over doing all three roles yourself in one context — the isolation is what actually
prevents the "just tweak the test" failure mode this law exists to stop.
