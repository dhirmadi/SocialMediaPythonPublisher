...
- Admin mode for **browser sessions** requires the signed, tenant/host-bound admin cookie (`pv2_admin`); machine clients use HTTP auth (`WEB_AUTH_TOKEN` Bearer or `WEB_AUTH_USER`/`WEB_AUTH_PASS` Basic). By default a valid cookie alone satisfies `require_auth` (#91 SEC-3 decision b); set `WEB_REQUIRE_HEADER_AUTH_WITH_COOKIE=1` to additionally require header auth for cookie sessions when a header backend is configured.
