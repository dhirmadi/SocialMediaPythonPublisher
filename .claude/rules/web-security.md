...
- Strict mode is enforced **inside `require_admin` itself** (PUB-048), not only in `require_auth`: with `WEB_REQUIRE_HEADER_AUTH_WITH_COOKIE=1` and a header backend configured, a valid admin cookie without a *verifying* `Authorization` header gets 401 on every `require_admin` call site — including the read-only view-permission check in `app.py` (`verify_view_permissions`, ~`app.py:476`) and `GET`/`POST /api/config/voice-profile`, which previously called `require_admin` alone. The check runs **after** the cookie check (no/invalid cookie stays 403 "Admin privileges required", not 401) and **re-verifies** the header via `_verify_bearer`/`_verify_basic` rather than merely testing that strict mode is configured — otherwise a valid header plus a valid cookie would be rejected. `require_auth`'s own strict-mode check stays; the one in `require_admin` is additive.
